...
from pytorch_influence_functions.influence_function import s_test, grad_z, avg_s_test
from pytorch_influence_functions.utils import save_json, display_progress



def calc_s_test(model, test_loader, train_loader, save=False, device=torch.device('cpu'),
                damp=0.01, scale=25, recursion_depth=5000, r=1, start=0,sample_num=1):
    ...
    for i in range(start, len(test_loader.dataset)):
        ...
        s_test_vec = calc_s_test_single(model, z_test, t_test, train_loader,
                                        device, damp, scale, recursion_depth, r, sample_num=sample_num)
...
def avg_calc_s_test_single(model, test_loader, train_loader, device=torch.device('cpu'),
                       damp=0.01, scale=25, recursion_depth=5000, r=1, sample_num=1):
    """Calculates s_test for a single test image taking into account the whole
    training dataset. s_test = invHessian * nabla(Loss(test_img, model params))

    Arguments:
        model: pytorch model, for which s_test should be calculated
        test_loader: test samples for averaging
        train_loader: pytorch dataloader, which can load the train data
        device: torch.device
        damp: float, influence function damping factor
        scale: float, influence calculation scaling factor
    ...
    s_test_vec_list = []
    for i in range(r):
        s_test_vec_list.append(avg_s_test(test_loader, model, train_loader,
                                      device=device, damp=damp, scale=scale,
                                      recursion_depth=recursion_depth,sample_num=sample_num))
...
def calc_s_test_single(model, z_test, t_test, train_loader, device=torch.device('cpu'),
                       damp=0.01, scale=25, recursion_depth=5000, r=1, sample_num=1):
    """Calculates s_test for a single test image taking into account the whole
    training dataset. s_test = invHessian * nabla(Loss(test_img, model params))
...
def calc_grad_z(model, train_loader, save_pth=False, device=torch.device('cpu'), start=0):
    ...
    for i in range(start, len(train_loader.dataset)):
        ...
        grad_z_vec = grad_z(z, t, model, device=device)
        if save_pth:
    ...
    for i in range(train_dataset_size):
        ...
        tmp_influence = sum(
            [
                ...
                # on the CPU or if device would work, too
                ###################################
...
def avg_calc_influence_single(model, train_loader, test_loader, device,
                          recursion_depth, r, s_test_vec=None,
                          time_logging=False, training_data_index=None, sample_num=1):
    ...
    Arugments:
        ...
        training_data_index: int, id of the train sample for which to calculate the
            influence function
        device: int, identifies the device id, -1 for cpu
        recursion_depth: int, number of recursions to perform during s_test
            calculation, increases accuracy. r*recursion_depth should equal the
            training dataset size.
        r: int, number of iterations of which to take the avg.
    ...
    s_test_vec = avg_calc_s_test_single(model, test_loader, train_loader,
                                        device, recursion_depth=recursion_depth,
                                        r=r,sample_num=sample_num)
    ...
    if training_data_index is None:
        train_index_size = train_dataset_size
        training_data_index = list(range(train_index_size))
    else:
        train_index_size = len(training_data_index)
    
    for idx in tqdm(range(train_index_size), desc="Calc. influence function"):
        i = training_data_index[idx]
        z, t, *others = train_loader.dataset[i]
        z = train_loader.collate_fn([z])
        t = train_loader.collate_fn([t])
        if time_logging:
    ...
    return influences, harmful.tolist(), helpful.tolist(), None, None # For alignment, but here pred and test_id is useless 


def calc_influence_single(model, train_loader, test_loader=None, test_id_num=None, device=torch.device('cpu'),
                          recursion_depth=5000, r=10, s_test_vec=None,
                          time_logging=False, training_data_index=None,sample_num=1):
    """Calculates the influences of all training data points on a single
    test dataset image.
    ...
    Arugments:
        model: pytorch model
        train_loader: DataLoader, loads the training dataset
        test_loader: DataLoader, loads the test dataset; if s_test_vec provided, test_load can be None
        test_id_num: int, id of the test sample for which to calculate the
            influence function; if s_test_vec provided, test_load can be None
        device: int, identifies the device id, -1 for cpu
        recursion_depth: int, number of recursions to perform during s_test
            calculation, increases accuracy. r*recursion_depth should equal the
            training dataset size.
    ...
    Returns:
        ...
        harmful: list of float, influences sorted by harmfulness
        helpful: list of float, influences sorted by helpfulness
        test_id_num: int, the id number of the test dataset point
            the influence was calculated for"""
    # Calculate s_test vectors if not provided
    assert s_test_vec is not None or test_loader is not None
    if not s_test_vec:
        z_test, t_test = test_loader.dataset[test_id_num][:2]
        z_test = test_loader.collate_fn([z_test])
        t_test = test_loader.collate_fn([t_test])
        s_test_vec = calc_s_test_single(model, z_test, t_test, train_loader,
                                        device, recursion_depth=recursion_depth,
    ...
    if z_test is not None:
        ...
        pred = F.softmax(model(z_test.to(device)).cpu(), dim=1)
        pred = pred.tolist()
    else:
        pred = None

    return influences, harmful.tolist(), helpful.tolist(), pred, test_id_num
...
def get_dataset_sample_ids(num_samples, test_loader, num_classes=None,
    ...
    for i in tqdm(range(num_classes), desc='get_dataset_sample_ids'):
...
def avg_calc_img_wise(config, model, train_loader, test_loader, training_data_index=None, top_num=500, save=True):
    ...
    test_sample_num_per_class = config['test_sample_num_per_class']
    ...
    if save:
        logging.info(f"Running on test dataset with {len(test_loader.dataset)} images for averaging.")
        influences_meta_fn = f"influences_results_meta_{test_start_index}-" \
                            f"{test_sample_num_per_class}.json"
    ...
    influence, harmful, helpful, *others = avg_calc_influence_single(
        model, train_loader, test_loader, device=config['device'],
    ...
    ###########
    influences['avg'] = {}
    influences['avg']['time_calc_influence_s'] = end_time - start_time
    infl = [x.cpu().numpy().tolist() for x in influence]
    influences['avg']['influence'] = infl
    influences['avg']['harmful'] = harmful[:top_num]
    influences['avg']['helpful'] = helpful[:top_num]
        
    #hvp_cache_path = outdir.joinpath(f"test{i}_hcp_cache.pt")
    #torch.save()
    #logging.info(f'The hvp .pt result of test sample {i} is saved at \'{hvp_cache_path}\'')

    logging.info(f"The results for this run are:")
    logging.info("Most harmful img IDs: ")
    logging.info(harmful[:5] if training_data_index is None else [training_data_index[harmful[i]] for i in range(5)])
    logging.info("Most helpful img IDs: ")
    logging.info(helpful[:5] if training_data_index is None else [training_data_index[helpful[i]] for i in range(5)])

    if save:
        influences_path = outdir.joinpath(f"influence_results_{test_start_index}_"
                                        f"{test_sample_num_per_class}.json")
        logging.info(f'The .json tmp result is saved at \'{influences_path}\'')
        save_json(influences, influences_path)

    return influences

def calc_img_wise(config, model, train_loader, test_loader, training_data_index=None):
    """Calculates the influence function one test point at a time. Calcualtes
    the `s_test` and `grad_z` values on the fly and discards them afterwards.
    ...
    # also be set to `0`.
    if test_sample_num_per_class and test_start_index is not False:
        test_dataset_iter_len = test_sample_num_per_class * config['num_classes']
        _, sample_list = get_dataset_sample_ids(test_sample_num_per_class, test_loader,
                                                    config['num_classes'],
                                                    test_start_index)
    else:
    ...
    # Set up logging and save the metadata conf file
    logging.info(f"Running on: {test_sample_num_per_class} images per class.")
    ...
    for j in range(test_dataset_iter_len):
        ...
        if test_sample_num_per_class and test_start_index:
        ...
        start_time = time.time()
        influence, harmful, helpful, pred, *others = calc_influence_single(
            model, train_loader, test_loader, test_id_num=i, device=config['device'],
            recursion_depth=config['recursion_depth'], r=config['r_averaging'],
            training_data_index=training_data_index)
        end_time = time.time()
        ...
        ###########
        influences[str(i)] = {}
        _, label, *others = test_loader.dataset[i]
        if len(others) == 3:
            original_index, _, original_label = others
        else:
            original_index, original_label = None, label
        influences[str(i)]['label'] = str(label)
        influences[str(i)]['original_label'] = str(original_label)
        influences[str(i)]['original_index'] = str(original_index) if original_index is not None else None
        influences[str(i)]['pred'] = pred
        ...
        tmp_influences_path = outdir.joinpath(f"influence_results_tmp_"
                                              ...
                                              f"{test_sample_num_per_class}"
                                              f"_last-i_{i}.json")
        save_json(influences, tmp_influences_path)
        logging.info(f'The .json tmp result is saved at \'{tmp_influences_path}\'')
...
def calc_all_grad_then_test(config, model, train_loader, test_loader):
    ...
    calc_s_test(model, test_loader, train_loader, s_test_outdir,
                config['device'], config['damp'], config['scale'],
                config['recursion_depth'], config['r_averaging'],
                config['test_start_index'])
    calc_grad_z(model, train_loader, grad_z_outdir, config['device'],
                config['test_start_index'])
