...
def adjust_learning_rate(optimizer, epoch, lr):
    if epoch & (epoch - 1) == 0: # the learning rate divided by 10 every 2 epoch, according to the original paper
        lr /= 10
...
class NADModelTrainer(PureCleanModelTrainer):
    ...
    def train_epoch(self,args,trainloader,nets,optimizer,scheduler,criterions,epoch):
        ...
        for idx, (inputs, labels, original_index, poison_indicator, original_targets, *other_info) in enumerate(trainloader):
            ...
            elif args.model in ['vgg19', 'vgg11', 'vgg16']:
                outputs_s = snet(inputs)
                features_out_3 = list(snet.children())[:-1]  # Remove the fully connected layer
                modelout_3 = nn.Sequential(*features_out_3)
                modelout_3.to(args.device, non_blocking=self.non_blocking)
...
class nad(defense):
    ...
    def set_result(self, result_file):
        attack_file = 'record/' + result_file  + '/' + args.result_name
