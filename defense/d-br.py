...
def train_epoch(arg, trainloader, model, optimizer, scheduler, criterion, epoch):
    ...
    for i, (inputs, labels, _, isCleans, gt_labels, *other_info) in enumerate(trainloader):
...
def finetune_epoch(arg, trainloader, model, optimizer, scheduler, epoch):
    ...
    for i, (inputs, labels, _, is_bd, gt_labels, *other_info) in enumerate(trainloader):
        inputs = normalization(arg, inputs[0])  # Normalize
        inputs, labels, gt_labels = inputs.to(arg.device), labels.to(arg.device), gt_labels.to(arg.device)
...
class d_br(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--clean_ratio', type=float, default=0.2, help='ratio of clean data') # \alpha_c
        parser.add_argument('--poison_ratio', type=float, default=0.4, help='ratio of poisoned data') # \alpha_p
    ...
    def set_result(self, result_file):
        ...
        self.result = load_attack_result(attack_file + f'/{args.result_name}')
    ...
    def mitigation(self):
        ...
        ###e. separate training samples into clean samples D_c, poisoned samples D_p, and uncertain samples D_u.
        clean_idx_list, poison_idx_list, suspicious_idx_list = separate_samples(args, bd_trainloader, model)
        
        ### e.0. get the fig of outlier score
        f_path = os.path.join(args.save_path, 'data_produce')
        f_all = os.path.join(f_path,'all.txt')

        all_data = np.loadtxt(f_all)
        
        ###e.1. logging the composition of clean_idx_list, poison_idx_list and suspicious_idx_list
        train_indicator = bd_trainloader.dataset.poison_indicator
        ...
        '''
        clean_len = len(clean_index)
        print(np.mean(all_data[bd_index]), np.mean(all_data[clean_index]), np.mean(all_data[cv_index]) if cv_index.shape else 0)
        assert 0
        bd_outlier_score = all_data[bd_index]
        cl_outlier_score = np.sort(all_data[clean_index])[clean_len//2-500:clean_len//2+500]
        cv_outlier_score = all_data[cv_index] if cv_index.shape else []
        
        outlier_info = [(sc, 0) for sc in cl_outlier_score] + [(sc, 1) for sc in bd_outlier_score] + [(sc, 2) for sc in cv_outlier_score]
        fig_path = os.path.join(args.save_path, 'outlier_fig.png')
        outlier_info.sort(key=lambda x: x[0], reverse=True)
        
        outlier_score, outlier_type = zip(*outlier_info)
        
        cl_color = (229/255, 133/255, 93/255)
        bd_color = (68/255, 117/255, 122/255)
        cv_color = (238/255, 213/255, 183/255)
        
        colors_map = {0:cl_color, 1:bd_color, 2:cv_color}
        
        outlier_colors = [colors_map[i] for i in outlier_type] 
        outlier_index = range(len(outlier_score))
        
        import matplotlib.pyplot as plt
        #rects = plt.bar(range(len(a)),b,width=0.3,color=['r','g','b','r','g','b'])
        plt.rcParams['font.sans-serif'] = ['SimHei']
        plt.rcParams['axes.unicode_minus'] = False 
        plt.bar(outlier_index, outlier_score, 1, color=outlier_colors) 
        plt.xlabel('Sample Index')  
        plt.ylabel('Outlier Score') 
        plt.title('') 
        plt.yticks(np.arange(0, np.max(outlier_score) + 0.2, 1)) 
        plt.xticks(np.arange(0.5, len(outlier_score) + 1.5, 200))
        plt.legend(loc="upper right") 
        plt.savefig('fig.png')
        '''
        cl_indecator = train_indicator[clean_idx_list]
        cl_clean_index = np.array(np.where(cl_indecator==0)).squeeze()
        cl_bd_index = np.array(np.where(cl_indecator==1)).squeeze()
        cl_cv_index = np.array(np.where(cl_indecator==2)).squeeze()
                    
        poi_indecator = train_indicator[poison_idx_list]
        poi_clean_index = np.array(np.where(poi_indecator==0)).squeeze()
        poi_bd_index = np.array(np.where(poi_indecator==1)).squeeze()
        poi_cv_index = np.array(np.where(poi_indecator==2)).squeeze()
        
        sus_indecator = train_indicator[suspicious_idx_list]
        sus_clean_index = np.array(np.where(sus_indecator==0)).squeeze()
        sus_bd_index = np.array(np.where(sus_indecator==1)).squeeze()
        sus_cv_index = np.array(np.where(sus_indecator==2)).squeeze()
        
        cl_clean_len = cl_clean_index.shape[0] if cl_clean_index.shape else 0
        cl_bd_len = cl_bd_index.shape[0] if cl_bd_index.shape else 0
        cl_cv_len = cl_cv_index.shape[0] if cl_cv_index.shape else 0
        poi_clean_len = poi_clean_index.shape[0] if poi_clean_index.shape else 0
        poi_bd_len = poi_bd_index.shape[0] if poi_bd_index.shape else 0
        poi_cv_len = poi_cv_index.shape[0] if poi_cv_index.shape else 0
        sus_clean_len = sus_clean_index.shape[0] if sus_clean_index.shape else 0
        sus_bd_len = sus_bd_index.shape[0] if sus_bd_index.shape else 0
        sus_cv_len = sus_cv_index.shape[0] if sus_cv_index.shape else 0
        
        logging.info(f'The length of detected clean samples are {len(clean_idx_list)}, among which {cl_clean_len}/{len(clean_index)} are clean samples, {cl_bd_len}/{len(bd_index)} are backdoored samples, {cl_cv_len}/{len(cv_index)} are cover smaples')
        logging.info(f'The length of detected backdoored samples are {len(poison_idx_list)}, among which {poi_clean_len}/{len(clean_index)} are clean samples, {poi_bd_len}/{len(bd_index)} are backdoored samples, {poi_cv_len}/{len(cv_index)} are cover smaples')
        logging.info(f'The length of detected suspicious samples are {len(suspicious_idx_list)}, among which {sus_clean_len}/{len(clean_index)} are clean samples, {sus_bd_len}/{len(bd_index)} are backdoored samples, {sus_cv_len}/{len(cv_index)} are cover smaples')
        
        def writeDataToCsv(path:str, data:dict):
            import pandas as pd
            data = pd.DataFrame(data)
            data.to_csv(path)
        writeDataToCsv(os.path.join(self.args.save_path, 'filter.csv'), 
                        [
                            ['type', 'clean', 'backdoored', 'suspicious'],
                            ['total samples', len(clean_idx_list), len(poison_idx_list), len(suspicious_idx_list)],
                            ['cl', f'{cl_clean_len}/{len(clean_index)}', f'{poi_clean_len}/{len(clean_index)}', f'{sus_clean_len}/{len(clean_index)}'],
                            ['bd', f'{cl_bd_len}/{len(bd_index)}', f'{poi_bd_len}/{len(bd_index)}', f'{sus_bd_len}/{len(bd_index)}'],
                            ['cv', f'{cl_cv_len}/{len(cv_index)}', f'{poi_cv_len}/{len(cv_index)}', f'{sus_cv_len}/{len(cv_index)}']
                        ])
