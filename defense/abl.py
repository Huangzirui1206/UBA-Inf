...
def compute_loss_value(args, poisoned_data, model_ascent):
    ...
    for idx, (img, target,*other_info) in tqdm(enumerate(example_data_loader, start=0)):
...
def isolate_data(args, result, losses_idx):
    ...
    train_dataset = dataset_wrapper_with_transform(
            result['bd_train'].wrapped_dataset,
            tf_compose,
            None,
    )
    data_set_without_tran = result['bd_train'].wrapped_dataset
    data_set_isolate = train_dataset
    ...
    train_indicator = train_dataset.poison_indicator
    ...
    re_indecator = train_indicator[perm]
    re_clean_index = np.array(np.where(re_indecator==0)).squeeze()
    re_bd_index = np.array(np.where(re_indecator==1)).squeeze()
    re_cv_index = np.array(np.where(re_indecator==2)).squeeze()
    
    re_clean_len = re_clean_index.shape[0] if re_clean_index.shape else 0
    re_bd_len = re_bd_index.shape[0] if re_bd_index.shape else 0
    re_cv_len = re_cv_index.shape[0] if re_cv_index.shape else 0
        
    logging.info(f'The length of potential backdoored samples are {len(perm)}, among which {re_clean_len}/{clean_index.shape[0]} are clean samples, {re_bd_len}/{bd_index.shape[0]} are backdoored samples, {re_cv_len}/{cv_index.shape[0]} are cover smaples')
    
    def writeDataToCsv(path:str, data:dict):
        import pandas as pd
        data = pd.DataFrame(data)
        data.to_csv(path)
    writeDataToCsv(os.path.join(args.save_path, 'filter.csv'), 
                    [
                        ['total suspicious samples', len(perm)],
                        ['cl', f'{re_clean_len}/{len(clean_index)}'],
                        ['bd', f'{re_bd_len}/{len(bd_index)}'],
                        ['cv', f'{re_cv_len}/{len(cv_index)}']
                    ])
...
class abl(defense):
    ...
    def set_result(self, result_file):
        ...
        self.result = load_attack_result(attack_file + f'/{self.args.result_name}')
    ...
    def mitigation(self):
        ...
        if self.args.only_scan: # just test whether abl can isolate the potentially backdoored samples.
            return {}
    ...
    def pre_train(self, args, result):
        ...
        train_indicator = data_set_o.poison_indicator
        bd_train_dataset = copy.deepcopy(data_set_o)
        print(type(bd_train_dataset))
        bd_train_dataset.subset(np.array(np.where(train_indicator==1)).squeeze())
        cl_train_dataset = copy.deepcopy(data_set_o)
        cl_train_dataset.subset(np.array(np.where(train_indicator==0)).squeeze())
        train_bd_loader = torch.utils.data.DataLoader(bd_train_dataset, batch_size=self.args.batch_size, num_workers=self.args.num_workers,drop_last=False, shuffle=True,pin_memory=args.pin_memory)
        train_clean_loader = torch.utils.data.DataLoader(cl_train_dataset, batch_size=self.args.batch_size, num_workers=self.args.num_workers,drop_last=False, shuffle=True,pin_memory=args.pin_memory)
        ...
        for epoch in range(0, args.tuning_epochs):
            ...
            clean_train_loss_avg_over_batch, \
            bd_train_loss_avg_over_batch, \
            ra_train_loss_avg_over_batch, \
            _, \
            _, \
            test_ra = self.eval_step(
                model_ascent,
            ...
            agg({
                ...
                "clean_train_loss_avg_over_batch": clean_train_loss_avg_over_batch,
                "bd_train_loss_avg_over_batch": bd_train_loss_avg_over_batch,
                "ra_train_loss_avg_over_batch": ra_train_loss_avg_over_batch,
            })
            ...
            bd_loss_list.append(bd_train_loss_avg_over_batch)
            cl_loss_list.append(clean_train_loss_avg_over_batch)
            ...
            general_plot_for_epoch(
                {
                    ...
                    "Train Clean Loss": cl_loss_list,
                    "Train Backdoordoor Loss": bd_loss_list
                },
            ...
            general_plot_for_epoch(
                ...
                save_path=f"{args.save_path}pre_train_loss_comparison_plots.png",
                ylabel="percentage",
            )
    ...
    def train_step(self, args, train_loader, model_ascent, optimizer, criterion, epoch):
        ...
        for idx, (img, target, original_index, poison_indicator, original_targets, *other_info) in enumerate(train_loader, start=1):
