...
class FinePrune(defense):
    ...
    def set_args(self, parser):
        ...
        parser.add_argument("--prune_ratio", type=float, help = "how many percent total prune. in 0 to 1")
        parser.add_argument('--result_name', type=str, default='attack_result.pt',
                            help='the name the result file')
        return parser
    ...
    def process_args(self, args):
        ...
        defense_save_path = args.save_path + os.path.sep + "defense/fp"
    ...
    def prepare(self, args):
        ...
        '''
                ...
                '''
        self.attack_result = load_attack_result(self.args.save_path + os.path.sep + args.result_name)
    ...
    def defense(self):
        ...
        logging.info(f"get ran_idx for subset clean train dataset, (len={len(ran_idx)})")
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, *other) in enumerate(trainloader):
                ...
                _ = netC(inputs)
                if flag == 0:
                    activation = torch.zeros(torch.sum(result_mid, dim=[0]).shape).to(args.device)
                    flag = 1
        ...
        for num_pruned in range(0, int(len(seq_sort) * 0.2), int(len(seq_sort) * args.once_prune_ratio)):
            net_pruned = (netC)
        ...
        model_dict = last_net.cpu().state_dict()
        pruned_tensors = {}
        for key in list(model_dict.keys()):
            if '_orig' in key:
                base_key = key.replace('_orig', '')
                if base_key not in pruned_tensors.keys():
                    pruned_tensors[base_key] = {}
                pruned_tensors[base_key]['orig'] = model_dict[key]
                del model_dict[key]
            elif '_mask' in key:
                base_key = key.replace('_mask', '')
                if base_key not in pruned_tensors.keys():
                    pruned_tensors[base_key] = {}
                pruned_tensors[base_key]['mask'] = model_dict[key]
                del model_dict[key]
        for base_key in pruned_tensors.keys():
            mask = pruned_tensors[base_key]['mask']
            orig = pruned_tensors[base_key]['orig']
            model_dict[base_key] = mask.to(dtype=orig.dtype) * orig
            
        save_defense_result(
            model_name=args.model,
            num_classes=args.num_classes,
            model=model_dict,
            save_path=args.defense_save_path,
            data_path=self.attack_result['data_path'],
            img_size=self.attack_result['img_size'],
            clean_data=self.attack_result['clean_data'],
            bd_train=self.attack_result['bd_train'],
            bd_test=self.attack_result['bd_test']
        )
