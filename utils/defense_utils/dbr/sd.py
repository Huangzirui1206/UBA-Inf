...
print(os.getcwd())
from utils.defense_utils.dbr.dataloader_bd import normalization
...
def calculate_consistency(args, dataloader, model):
    ...
    model.eval()
    for i, (inputs, labels, _, is_bd, gt_labels, *others) in enumerate(dataloader):
...
def separate_samples(args, trainloader, model):
    ...
    poison_idx_list = []
    suspicious_idx_list = []    
    for i, (inputs, labels, original_index, _, gt_labels, *others) in enumerate(trainloader):
        if args.debug and i==10001:
            break
        print("Processing samples:", i*args.batch_size, '\r', end='')
        inputs1, inputs2 = inputs[0], inputs[2]
        ...
        ### Separate samples ###
        clean_idx_list += original_index[torch.where(feature_consistency <= gamma_low)[0].cpu()]
        poison_idx_list += original_index[torch.where(feature_consistency >= gamma_high)[0].cpu()]
        suspicious_idx_list += original_index[torch.where((feature_consistency > gamma_low) & (feature_consistency < gamma_high))[0].cpu()]
    print("Processing samples done.")
    ...
    return clean_idx_list, poison_idx_list, suspicious_idx_list

def separate_samples_back(args, trainloader, model):
    gamma_low, gamma_high = args.gamma_low, args.gamma_high
    model.eval()
    clean_samples, poison_samples, suspicious_samples = [], [], []
    feature_consistency_total = None
    for i, (inputs, labels, _, _, gt_labels, *others) in enumerate(trainloader):
        if args.debug and i==10001:
            break
        if i % 1000 == 0:
            print("Processing samples:", i, '\r', end='')
        inputs1, inputs2 = inputs[0], inputs[2]
        ...
        if feature_consistency_total is None:
            feature_consistency_total = feature_consistency
        else:
            feature_consistency_total = torch.concatenate((feature_consistency_total, feature_consistency))
        # feature_consistency = feature_consistency.detach().cpu().numpy()
    ...
    feature_consistency_total = torch.sort(feature_consistency_total)
    print('Processing samples done.')
