...
def calculate_consistency(args, dataloader, model):
    ...
    model.eval()
    for i, (inputs, labels, _, is_bd, gt_labels, *other_info) in enumerate(dataloader):
...
def separate_samples(args, trainloader, model):
    ...
    poison_idx_list = []
    suspicious_idx_list = []
    for i, (inputs, labels, original_index, _, gt_labels, *other_info) in enumerate(trainloader):
        if args.debug and i==10001:
            break
        print(f"Processing samples:{i*args.batch_size}\r")
        inputs1, inputs2 = inputs[0], inputs[2]
...
def separate_samples_back(args, trainloader, model):
    ...
    for i, (inputs, labels, _, _, gt_labels, *other_info) in enumerate(trainloader):
        if args.debug and i==10001:
            break
        if i % 1000 == 0:
            print(f"Processing samples:{i}\r", end='')
        inputs1, inputs2 = inputs[0], inputs[2]
