...
def save_attack_result(
    ...
    result_name : str = 'attack_result.pt'
):
    '''
...
def save_unlearn_result(model_name : str,
    num_classes : int,
    model : dict, # the state_dict
    save_path : str,
    data_path : str,
    ...
    logging.info(f"saving...")
    logging.debug(f"location : {save_path}/{save_name}_result.pt") #, content summary :{pformat(summary_dict(save_dict))}")

    torch.save(
        save_dict,
        f'{save_path}/{save_name}_result.pt',
    )
...
def load_attack_result(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        if load_file['bd_train'] is not None:
            ...
            logging.warning("No bd_train info found.")
        ...
        load_dict = {
                ...
                'img_size': load_file['img_size'],
                'num_classes': load_file['num_classes'],
                'clean_data': load_file['clean_data'],
                'data_path': load_file['data_path']
            }

        print(f"loading...")
        return load_dict
    
    elif all(key in load_file for key in ['num_classes',
        'img_size',
        'clean_data',
        'bd_train',
        'bd_test',
