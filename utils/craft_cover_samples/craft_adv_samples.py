'''
This is for crafted the covering samples.
'''
...
class PGD(Attack):
    ...
    def forward(self, images, labels):
        ...
        if self.random_start:
            ...
            adv_images = torch.clamp(adv_images, min=-0.5, max=0.5) # considering the nomalization
...
def main():
    ...
    train_img_transform = Compose(
        [Resize((args.input_height,args.input_width)), ToTensor(), transforms.Normalize([0.5], [0.5])]
    )
    ...
    if args.dataset == 'cifar10':
        ...
        #net.load_state_dict(torch.load('../../data/models/resnet18_PGD_best_model.pth'))
        net.load_state_dict(torch.load('../../data/models/cifar10_resnet18_AT_best_model.pth'))
    elif args.dataset == 'tiny':
        net = PreActResNet18(num_classes=args.num_classes)
        net.load_state_dict(torch.load('../../data/models/preact_tiny_model.pth'))
    elif args.dataset == 'cifar100':
        net = ResNet18(num_classes = args.num_classes)
        net.load_state_dict(torch.load('../../data/models/cifar100_resnet18_AT_best_model.pth'))
    elif args.dataset == 'gtsrb':
        net = ResNet18(num_classes = args.num_classes)
        checkpoint = torch.load('../../data/models/gtsrb_resnet18_AT_best_model.pth')
        ...
        net = ResNet18(num_classes = args.num_classes)
        net.load_state_dict(torch.load('../../data/models/mnist_resnet18_AT_best_model.pth'))
    elif args.dataset == 'fashion-mnist':
        net = ResNet18(num_classes = args.num_classes)
        net.load_state_dict(torch.load('../../data/models/fashion_mnist_resnet18_AT_best_model.pth'))
    ...
    config = {"adv_dataset_dir": "../../data/adv_samples",#"./test/adv_dataset",
    ...
    attacker.set_return_type("float")
    ...
    for (x,y,*other) in tqdm(train_loader):
        ...
        img = attacker(x,y)
        adv_y = torch.argmax(net(transforms.Normalize([0.5], [0.5])(img.float())), dim=1)  
        perturbed_img[i: i + len(img), :, :, :] = img.permute(0, 2, 3, 1).detach()
        target[i: i + len(y)] = adv_y #y
        i += img.shape[0]
    ...
    adv_target_path = os.path.join(
        config["adv_dataset_dir"],f"{args.dataset}_train_y.npy"
    )
    # np.savez(adv_data_path, data=perturbed_img.numpy(), targets=target.numpy())
    np.save(adv_data_path, perturbed_img.numpy())
    #np.save(adv_target_path, target.numpy())
