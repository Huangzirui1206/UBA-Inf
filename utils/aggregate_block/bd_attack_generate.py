...
from utils.bd_img_transform.narcissus import narcissusTriggerAttack
...
from utils.unnormalize import UnNormalize, ToNumpy, PIL2Numpy
...
def bd_attack_img_trans_generate(args):
    ...
    if args.attack in ['badnet',]:
        ...
        noise_npy = np.load(args.narcissus_noise_path) 
        noise = torch.from_numpy(noise_npy)
        
        ### The same backdoor application as that in official code
        train_trans = narcissusTriggerAttack(
            noise=noise,
            offset_x=args.offset_x, offset_y=args.offset_y,
            mode=args.mode, padding=args.padding, position=args.position    
        )
        
        test_trans = narcissusTriggerAttack(
            noise=noise * args.multi_test,
            offset_x=args.offset_x, offset_y=args.offset_y,
            mode=args.mode, padding=args.padding, position=args.position    
        )
        ...
        transfrom_ndarray = transforms.Compose([
            UnNormalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
            ToNumpy(),
        ])   
        
        logging.info(f"Load Narcissus trigger with shape {noise.shape} from path: {args.narcissus_noise_path}")
        
        train_bd_transform = general_compose([
            (transforms.Resize(args.img_size[:2]), False),
            (transform_tensor, False),
            (train_trans, False),
            (transfrom_ndarray, False),
            (npClipAndToUint8,False),
        ])
        ...
        raise NotImplementedError('No such attack implemented!')
...
def cv_attack_label_trans_generate(args):
    if args.cover_method == 'adversarial-label-consistent' or\
       args.cover_method == 'label-consistent' or\
       args.cover_method == 'adversarial-single-label-consistent' or\
       args.cover_method == 'single-label-consistent':
        return labelConsistent_cover()
    elif args.cover_method == 'const-label-flipping':
        return constLabelFlipping_cover(int(args.clabel))
    elif args.cover_method == 'random-label-flipping':
        return randomLabelFlipping_cover(int(args.num_classes))
    else:
        raise ValueError('No such cover method.')