...
def get_num_classes(dataset_name: str) -> int:
    ...
    if dataset_name in ["mnist", "cifar10", "fashion-mnist"]:
        num_classes = 10
...
def get_transform(dataset_name, input_height, input_width, train=True, random_crop_padding=4):
    ...
    if dataset_name in ['mnist', 'fashion-mnist'] or input_height < 32:
        padding = True
    else:
        padding = False
    ...
    if padding:
        # transform mnist image into 32 * 32 manner
        def add_padding(image):
            # Pad the image on both sides (top, bottom, left, right) with 2 pixels
            padded_image = torch.nn.functional.pad(image, (2, 2, 2, 2), mode='constant', value=0)
            return padded_image
        transforms_list.append(transforms.Lambda(add_padding))
...
def dataset_and_transform_generate(args):
    ...
    if not args.dataset.startswith('test'):
        ...
        train_img_transform = get_transform('test', *(args.img_size[:2]), train=True)
    ...
    if (train_dataset_without_transform is None) or (test_dataset_without_transform is None):
        ...
        if args.dataset.startswith('test'):  # for test only
            ...
            def add_channel(image):
                return Image.fromarray(np.repeat(np.expand_dims(image, 2), 3, 2))
            # Define the dataset with transform
            transform = transforms.Compose([
                transforms.Lambda(add_channel),  # Add channel dimension
            ])
            ...
            train_dataset_without_transform = FashionMNIST(
                args.dataset_path,
                train=True,
                transform=transform,
                download=True,
            )
            test_dataset_without_transform = FashionMNIST(
                args.dataset_path,
                train=False,
                transform=transform,
                download=True,
            ...
            logging.warning("Not support ImageNet yet.")
            logging.debug("Not support ImageNet yet.")
            assert 0
