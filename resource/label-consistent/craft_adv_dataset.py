...
class PGD(Attack):
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if self.gray:
                gray_tensor = torch.mean(adv_images, dim=1, keepdim=True)
                adv_images = torch.cat([gray_tensor] * 3, dim=1)
...
def main():
    ...
    config = {"adv_dataset_dir": "./data/adv_dataset",
    ...
    for (x,y,*other) in tqdm(train_loader):
        ...
        img = attacker(x,y)
        perturbed_img[i: i + len(img), :, :, :] = img.permute(0, 2, 3, 1)
        target[i: i + len(y)] = y
        i += img.shape[0]
