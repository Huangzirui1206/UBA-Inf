...
class SisaNormalCase:
    ...
    def stage0_training_data_split(self):
        # According to SISA algorithm, split the training dataset into shards
        train_dataset_without_transform, \
        train_img_transform, \
        train_label_transform, \
        test_dataset_without_transform, \
        ...
        shard_size = len(train_dataset_without_transform) // num_shards
        ...
        random_index = torch.randperm(len(train_dataset_without_transform)).tolist() # shuffle the training dataset
        
        for _ in range(num_shards):                        
            shard = Subset(train_dataset_without_transform, random_index[start_idx: end_idx]) # Get data shard
            shards.append(shard)
            start_idx = end_idx
            end_idx += shard_size
        
        for shard in shards:
            clean_train_datashard_with_transform = dataset_wrapper_with_transform(
                shard,
                train_img_transform,
                train_label_transform
            )
            clean_train_datashard_targets = get_labels(shard)
            ...
            yield shard, \
                ...
                test_img_transform, \
                test_label_transform, \
                clean_train_datashard_with_transform, \
                clean_train_datashard_targets, \
                clean_test_dataset_with_transform, \
                clean_test_dataset_targets
    ...
    def stage1_non_training_data_prepare(self):
        ...
        # Outside: self.data_prepare = self.stage0_training_data_split()
        train_dataset_without_transform, \
        train_img_transform, \
        train_label_transform, \
        test_dataset_without_transform, \
        ...
        clean_test_dataset_with_transform, \
        clean_test_dataset_targets \
            = next(self.data_prepare) # get the dataset of a shard
    ...
    def stage2_training(self):
        ...
        # Since only generate one shard model at once, modify the save_path as a subdir
        save_path = args.save_path + "/shard" + str(self.shard_cnt)
        self.shard_cnt += 1
        ...
        torch.save(self.net.cpu().state_dict(), f"{save_path}/clean_model.pth")


if __name__ == '__main__':
    normal_train_process = SisaNormalCase()
